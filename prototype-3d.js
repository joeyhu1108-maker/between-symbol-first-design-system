...
import {backendReady,createJob,waitForJob,mountPrinterScene,submitPrint} from './bridge.js';
...
async function finishPrint(){const job=state.generation.job,print=await state.print,card=CARDS[state.answerId],queued=print?.print_status==='submitted';setStage('done');showPrintProgress(1);$('printTitle').textContent='✓';$('restart').hidden=false;if(!job){$('printTitle').setAttribute('aria-label','作品完成，可以取出');$('printCopy').textContent=`${card.id} · ${card.name} 已发送至小米打印机。`;return}$('layerText').textContent=queued?'✓':'×';$('printTitle').setAttribute('aria-label',queued?'作品已交给打印机，请取件':'作品已归档，打印未完成');$('printCopy').textContent=`${card.id} · ${card.name} · ${job.id} `+(queued?`已交给打印机 ${print.printer}（队列 ${print.cups_job_id}），请到打印机取件。`:print?.print_status==='no_printer'?'已归档；没有找到纸张打印机。':`已归档；打印未完成：${print?.print_error||print?.print_status||'未提交'}`)}
