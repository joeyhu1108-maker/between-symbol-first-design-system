#!/usr/bin/env python3
"""Local, persistent Chladni artwork jobs and explicit CUPS printing. No cloud dependency."""
import json, math, os, re, secrets, sqlite3, subprocess, threading, time
...
INPUT=ROOT.parent.parent/'卡片'/'种子显影台.html'
...
def params(data):
    ...
    a,b=float(data.get('a',.8)),float(data.get('b',.6))
...
def generate(jid):
    ...
    try:
        ...
        pdf.setTitle(jid); pdf.setAuthor('星环中的萌生 / Chladni rule generator')
        ...
        job.update(status='ready',ready_at=datetime.now().isoformat(),image=f'/jobs/{jid}/artwork.png',pdf=f'/jobs/{jid}/artwork.pdf',particles=f'/jobs/{jid}/particles.json',particle_count=N)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path=='/input':
                raw=INPUT.read_bytes(); self.send_response(200); self.send_header('Content-Type','text/html; charset=utf-8'); self.end_headers(); return self.wfile.write(raw)
            if path.endswith(('.sqlite3','.py','.log')): return self.send_error(403)
            return super().do_GET()
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.send_error(404)
        except (ValueError,TypeError,OverflowError) as e: self.send_json({'error':str(e)},400)
