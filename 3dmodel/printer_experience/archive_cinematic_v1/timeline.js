...
export function timeline(elapsed,{mode='rehearsal',readyAt=null,failed=false}={}){
 ...
 const creationStart=available?Math.max(8.6,readyAt+.35):Infinity;
 ...
 const phase=elapsed<7.9?'assembly':elapsed<creationStart?'assembled':elapsed<printEnd?'creating':elapsed<finishAt?'print':'complete';
 return {phase,elapsed,creationStart,printStart,printEnd,finishAt,reveal:creationStart,form:smooth(.8,7.9,elapsed),bloom:creation,landing:feed,print:feed,creation,explosion:0,complete:available&&elapsed>=finishAt,overdue:elapsed>30&&!available};
}
