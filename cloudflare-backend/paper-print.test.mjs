...
function harness({nfc=true,paired=true,ready=true,send=async()=>({id:'original-run',received_at:'now'})}={}){
  const elements=new Map(),calls=[],events=[],job={id:'SG-20260913-0053-12345678'};
  ...
  return {...context.api,state,job,calls,events,element,async flush(){for(let i=0;i<12;i++)await Promise.resolve();}};
}
