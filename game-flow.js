...
function artworkLink(id,card){const link=$(id);if(!link)return;link.hidden=!card;if(card)link.href=`./prototype-3d.html?card=${card.id}`;else link.removeAttribute('href');}
function clearResult(){if($('game-result'))$('game-result').hidden=true;setText('game-result-copy','');artworkLink('result-artwork-link',null);}
...
function reveal(card,source='wheel'){
  ...
  artworkLink('card-artwork-link',card);clearResult();
}
...
function drawCard(){
  ...
  artworkLink('card-artwork-link',null);clearResult();if($('card-scan-link'))$('card-scan-link').hidden=true;
...
function finish(){
  ...
  if($('game-result')){$('game-result').hidden=false;setText('game-result-copy',`${game.card.id} · ${game.card.name} — ${game.card.result}`);artworkLink('result-artwork-link',game.card);}
...
function leave(){
  ...
  setText('draw-card','让小球选择');$('draw-card').disabled=false;$('enter-node').disabled=true;clearResult();artworkLink('card-artwork-link',null);if($('card-scan-link'))$('card-scan-link').hidden=true;window.dispatchEvent(new CustomEvent('seed-game-leave'));$('draw-card').focus({preventScroll:true});
}
